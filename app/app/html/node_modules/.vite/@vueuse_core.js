...
function set2(...args) {
  ...
  if (args.length === 3) {
    if (isVue2) {
      (init_index_esm(), index_esm_exports).set(...args);
    } else {
...
function toRefs2(objectRef) {
  ...
  const ret = Array.isArray(objectRef.value) ? new Array(objectRef.value.length) : {};
  ...
  const execute = async () => {
    ...
    if (config.payload) {
      ...
      if (config.payloadType === "json") {
        ...
        headers["Content-Type"] = config.payloadType === "formData" ? "multipart/form-data" : "text/plain";
      }
    }
    ...
    return new Promise((resolve) => {
      var _a;
      fetch(context.url, Object.assign(Object.assign(Object.assign({}, defaultFetchOptions), context.options), { headers: Object.assign(Object.assign({}, defaultFetchOptions.headers), (_a = context.options) === null || _a === void 0 ? void 0 : _a.headers) })).then(async (fetchResponse) => {
...
function useMousePressed(options = {}) {
  const { touch = true, initialValue = false, window: window2 = defaultWindow } = options;
...
function useRefHistory(source, options = {}) {
  ...
  const { ignoreUpdates, ignorePrevAsyncUpdates, stop: stop2 } = ignorableWatch(source, commit, { deep, flush, eventFilter });
